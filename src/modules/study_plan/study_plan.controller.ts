...
import {
  UpdateMutipleStudyPlansDto,
  UpdateStudyPlanDto,
} from './dtos/updateStudyPlan.dto';
...
export class StudyPlanController {
  ...
  async create(
    ...
    return new SuccessResponse({
      ...
      data: studyPlan,
      message: 'Đăng kế hoạch học tập thành công',
    }).send(res);
  }
  ...
  async cancelMutiple(
    @Body() dto: UpdateMutipleStudyPlansDto,
    @Req() req: RequestHasUserDto & Request,
    @Res() res: Response,
  ) {
    const currentUser = req.user;
    const { studyPlanIds } = dto;
    const studyPlan = await this.studyPlanService.cancelMultiple(
      studyPlanIds,
      currentUser,
    );
    return new SuccessResponse({
